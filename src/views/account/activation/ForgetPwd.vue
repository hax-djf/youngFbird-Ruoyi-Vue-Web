                ...
                >
                    <el-tab-pane label="手机找回" name="phone">
                        <el-form ref="phoneForm" :model="retrieveAccountForm" :rules="phoneRules" label-width="0px">
                            ...
                            <el-form-item>
                                <el-button class="login-submit" style="width:100%;" type="primary"  @click="phoneRetrievePassWordHandle">
                                    找回密码
                                </el-button>
                            </el-form-item>
                        </el-form>
                    ...
                    <el-tab-pane label="邮箱找回" name="email">
                        ...
                        >
                            ...
                            <el-form-item>
                                <el-button class="login-submit" style="width:100%;" type="primary" @click="sendEmailValidateHandle">
                                    找回密码
                                </el-button>
                            </el-form-item>
                        </el-form>
        ...
        <div v-if="retrieveStep===2" class="reset-pwd-view">
            <div>
                ...
                <el-form ref="resetPwdForm" :model="resetPwdForm" :rules="pwdRules" label-width="0px">
                    ...
                    <el-form-item>
                        <el-button class="login-submit" style="width:100%;" type="primary" @click="resetPasswordHandle">
                            提交
                        </el-button>
                    </el-form-item>
                </el-form>
...
import {retrieveCodeByPhone,retrieveCodeByEmail,retrieveCheckPhone,retrieveReset} from '@/api/system/forgetpwd'
export default {
    name: 'RetrievePwd',
    data() {
    ...
    methods: {
        ...
        sendPhoneValidateCode() {
            ...
            this.$refs['phoneForm'].validateField('phoneNumber', err => {
                if (!err) {
                    ...
                    retrieveCodeByPhone(params).then(() => {
        ...
        phoneRetrievePassWordHandle() {
            this.$refs['phoneForm'].validate(valid => {
                if (valid) {
                    retrieveCheckPhone(this.retrieveAccountForm).then(res => {
                        if (res.data) {
        ...
        resetPasswordHandle() {
            this.$refs['resetPwdForm'].validate(valid => {
                if (valid) {
                    retrieveReset(this.resetPwdForm).then(res => {
                        if (res.data) {
                            this.msgSuccess('密码重置成功，快去登录吧')
                            setTimeout(() => {
                                this.$router.push({ path: '/official/login' })
                            }, 2000)
                        }
                    })
                }
        ...
        sendEmailValidate() {
            this.$refs['emailForm'].validate(valid => {
                if (valid) {
                    let params =  { email: this.retrieveAccountForm.email };
                    retrieveCodeByEmail(params).then(() => {
                        this.retrieveStep = 3
                    })
                }
            })
